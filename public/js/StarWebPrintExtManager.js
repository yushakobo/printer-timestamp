...
StarWebPrintExtManager.prototype._analyzeXml = function (a, d) {
    var e = new DOMParser, b, f; b = e.parseFromString(a.responseText, "text/xml"); f = b.getElementsByTagName("Response"); b = e.parseFromString(f.item(0).textContent, "text/xml"); b.getElementsByTagName("root"); f = b.getElementsByTagName("claimid").item(0); var g = b.getElementsByTagName("claim").item(0), e = b.getElementsByTagName("event"); a.managerClaim = g.textContent; a.managerSuccess = a.traderSuccess; a.managerCode = a.traderCode; a.traderSuccess = void 0; a.traderCode =
        void 0; a.traderStatus = void 0; if ("true" == a.managerSuccess) {
            if ("true" == a.managerClaim) {
                void 0 != f && (this.claimId = f.textContent, this.isPolling = !0); var c = this; f = {
                ...
                }; g = 0; if (this.isPolling || this.isWaitWriteCallback) {
                    if (void 0 != b.getElementsByTagName("eventcount")) try { g = b.getElementsByTagName("eventcount").item(0).textContent } catch (m) { } for (b =
                        0; b < g; b++)for (var h = 0; h < e.length; h++) { var k = e.item(h).getElementsByTagName("action").item(0).textContent; if (e.item(h).getElementsByTagName("number").item(0).textContent == b) if ("onBarcodeDataReceive" == k || "onStatusUpdate" == k) { var l = e.item(h).getElementsByTagName("data").item(0).textContent; try { f[k](l) } catch (n) { } } else try { f[k]() } catch (p) { } }
                }
            }
        } else 1200 != a.managerCode && (2001 != a.managerCode && !(d && 1010 == a.managerCode)) && this._clearClaimId(); return a
};
