...
//
class StarBarcodeEncoder {
	constructor() { }
	createUPCE(a) {
		var f = "0001101 0011001 0010011 0111101 0100011 0110001 0101111 0111011 0110111 0001011".split(" "), e = "0100111 0110011 0011011 0100001 0011101 0111001 0000101 0010001 0001001 0010111".split(" ");
		a = a.data;
		if (!(11 == a.length || 12 == a.length))
			throw Error("Data length is invalid.");
		if (a.match(/[^0-9]/g))
			throw Error("Data is invalid.");
		12 == a.length && (a = a.substring(0, 11));
		for (var c = 0, b = 0; 11 > b; b++)
			c = b % 2 ? c + parseInt(a.charAt(b)) : c + 3 * parseInt(a.charAt(b));
		if (c %=
			10)
			c = 10 - c;
	...
	createUPCA(a) {
		var f = "0001101 0011001 0010011 0111101 0100011 0110001 0101111 0111011 0110111 0001011".split(" "), e = "1110010 1100110 1101100 1000010 1011100 1001110 1010000 1000100 1001000 1110100".split(" ");
		a = a.data;
		if (!(11 == a.length || 12 == a.length))
			throw Error("Data length is invalid.");
		if (a.match(/[^0-9]/g))
		...
		if (c %= 10)
			c = 10 - c;
		for (var d = "101", b = 0; 6 > b; b++)
			d +=
				f[parseInt(a.charAt(b))];
		d += "01010";
		for (b = 6; 11 > b; b++)
			d += e[parseInt(a.charAt(b))];
		d += e[c];
		return d + "101";
	}
	createJAN8(a) {
		...
		if (a.match(/[^0-9]/g))
			throw Error("Data is invalid.");
		for (var c = 0, b = 0; 7 > b; b++)
			c = b % 2 ? c + parseInt(a.charAt(b)) : c + 3 * parseInt(a.charAt(b));
		if (c %= 10)
			c = 10 - c;
		for (var d = "101", b = 0; 4 > b; b++)
			d +=
				f[parseInt(a.charAt(b))];
		d += "01010";
		for (b = 4; 7 > b; b++)
	...
	createJAN13(a) {
		var f = "0001101 0011001 0010011 0111101 0100011 0110001 0101111 0111011 0110111 0001011".split(" "), e = "0100111 0110011 0011011 0100001 0011101 0111001 0000101 0010001 0001001 0010111".split(" "), c = "1110010 1100110 1101100 1000010 1011100 1001110 1010000 1000100 1001000 1110100".split(" ");
		a = a.data;
		if (!(12 == a.length || 13 == a.length))
			throw Error("Data length is invalid.");
		if (a.match(/[^0-9]/g))
			throw Error("Data is invalid.");
		for (var b = 0, d = 0; 12 > d; d++)
			b =
				d % 2 ? b + 3 * parseInt(a.charAt(d)) : b + parseInt(a.charAt(d));
		if (b %= 10)
			b = 10 - b;
		for (var g = "101", h = "0000000 0001011 0001101 0001110 0010011 0011001 0011100 0010101 0010110 0011010".split(" ")[parseInt(a.charAt(0))], d = 1; 7 > d; d++)
			g = "0" == h[d] ? g + f[parseInt(a.charAt(d))] : g + e[parseInt(a.charAt(d))];
		g += "01010";
		for (d = 7; 12 > d; d++)
			g += c[parseInt(a.charAt(d))];
		g += c[b];
		return g + "101";
	}
	createCode39(a) {
		...
		if (a.match(/[^ \$\%\+\-\.\/0-9A-Z]/g))
			throw Error("Data is invalid.");
		for (var e = "NwNnWnWnNn", c = 0; c < a.length; c++)
			e += f[a.charCodeAt(c) - 32];
		return e + "NwNnWnWnN";
	}
	createITF(a) {
		...
		if (a.match(/[^0-9]/g))
			throw Error("Data is invalid.");
		a.length % 2 && (a = "0" + a);
		for (var c = "NnNn", b = 0; b < a.length; b += 2)
			var d = f[parseInt(a.charAt(b))], g = e[parseInt(a.charAt(b + 1))], c = c + d.charAt(0), c = c + g.charAt(0), c = c + d.charAt(1), c = c + g.charAt(1), c = c + d.charAt(2), c = c + g.charAt(2), c = c + d.charAt(3), c = c + g.charAt(3), c = c + d.charAt(4), c = c + g.charAt(4);
		return c + "WnN";
	}
	createCode128(a) {
		...
		switch (e.charAt(1)) {
			...
			case "C":
				c = 2;
				break;
			default: throw Error("Data is invalid.");
		}
		...
		for (var b = 2; b < e.length; b++) {
			...
			if ("{" == d) {
				...
				switch (c) {
					...
					default:
						if (0 <= g && 31 >= g)
							a += String.fromCharCode(g + 64);
						else if (32 <= g && 95 >= g)
							a += String.fromCharCode(g - 32);
						else
							throw Error("Data is invalid.");
						break;
					case 4: c = 0;
